...
class AxesDrawer
{
    ...
    func drawBasicCircle(bounds: CGRect, origin: CGPoint){
    ...
    private func drawHashmarksInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat)
    {
        if ((origin.x >= bounds.minX) && (origin.x <= bounds.maxX)) || ((origin.y >= bounds.minY) && (origin.y <= bounds.maxY))
        {
            ...
            while !bbox.contains(bounds)
            {
                ...
                let label = formatter.string(from: NSNumber(value: Double(origin.x-bbox.minX)/Double(pointsPerUnit)))
    ...
    private func alignedPoint(x: CGFloat, y: CGFloat, insideBounds: CGRect? = nil) -> CGPoint? //took out extraneous x
    {
        let point = CGPoint(x: align(coordinate: x), y: align(coordinate: y))
        //        if let permissibleBounds = insideBounds where !permissibleBounds.contains(point) { //expected joining of multi clause thing
